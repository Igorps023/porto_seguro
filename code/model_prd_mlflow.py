# %% [1] CONFIGURAÇÃO INICIAL (ANTES DE QUALQUER RUN)
import sys
import os
import mlflow
...
# Digitar este comando no cmd para iniciar o servidor mlflow ui --host 127.0.0.1 --port 5000
...
try:
    ...
    print(f"Experimento já existe: {e}")
...
param_dist_random = [
    {
        ...
        'clf__C': [0.001, 0.01, 0.1, 1, 10, 100], # Range amplo
        'clf__penalty': ['l1', 'l2']
    },
    {
        'clf': [RandomForestClassifier(random_state=42, class_weight='balanced')],
        'clf__n_estimators': randint(5, 15),
        'clf__max_depth': [5, 10, 15, 20, None],
        'clf__min_samples_leaf': randint(100, 5000)
    }
]
...
print("🤖 Ativando autolog para treinamento de modelos...")
mlflow.sklearn.autolog(
    log_models=True,
    log_model_signatures=True
)

with mlflow.start_run(run_name=f"random_search_{datetime.now().strftime('%Y%m%d_%H%M%S')}") as random_run:
    print(f"🔬 Executando Random Search... (Run: {random_run.info.run_id})")

    # Tags para organização
    mlflow.set_tag("phase", "random_search")
    mlflow.set_tag("purpose", "exploratory_tuning")
    ...
    random_search = RandomizedSearchCV(
        ...
        n_iter=10, # Testará 15 combinações aleatórias
        cv=2,
        scoring='roc_auc', 
        n_jobs=-1, 
        verbose=2,
    ...
    # Log manual apenas do que NÃO é coberto pelo autolog
    mlflow.log_metric("random_search_time", random_time)
    mlflow.set_tag("model_family", str(random_search.best_estimator_['clf'].__class__.__name__))
...
# %%
with mlflow.start_run(run_name=f"grid_search_final_{datetime.now().strftime('%Y%m%d_%H%M%S')}") as grid_run:
    # Tags para organização
    mlflow.set_tag("phase", "grid_search_final")
    mlflow.set_tag("purpose", "refinement_tuning")
    ...
    param_grid_refined = [
        {
            ...
            'clf__n_estimators': [300], #[100, 200, 300], #[10],
            'clf__max_depth': [15], #[12, 15, 18], #[10],
            'clf__min_samples_leaf': [1200], #[1500, 1785, 2000]
        },
        {
    ...
    # ============ AVALIAÇÃO (MANUAL - complementa o autolog) ============
    model_final = grid_search.best_estimator_
    mlflow.set_tag("final_model_type", str(model_final.named_steps['clf'].__class__.__name__))
    ...
    mlflow.log_metric("test_accuracy", metrics.accuracy_score(y_test, y_test_predict))
    ...
    # 1. LOG DO PREPROCESSOR (único item obrigatório não coberto pelo autolog)
    mlflow.sklearn.log_model(preprocessor, "preprocessor")

    # 2. LOG DAS FEATURES (de forma inteligente, não a lista completa)
    mlflow.log_param("best_features_count", len(best_features))
    if len(best_features) <= 10:
        mlflow.log_param("best_features", str(best_features))
    else:
        mlflow.log_param("top_5_features", str(best_features[:5]))
        # Salva lista completa como arquivo
        pd.DataFrame({"feature": best_features}).to_csv("best_features_list.csv", index=False)
        mlflow.log_artifact("best_features_list.csv")

    # 3. LOG DOS ARTEFATOS COMPLEMENTARES
    feature_importances.to_csv("feature_importances.csv", index=False)
    mlflow.log_artifact("feature_importances.csv")

    # 4. LOG DA CURVA ROC (apenas UMA vez, remova a duplicação)
    import matplotlib.pyplot as plt
    from sklearn.metrics import roc_curve, classification_report

    plt.figure(dpi=400, figsize=(8, 6))
    plt.plot(roc_train[0], roc_train[1], linewidth=2, label=f"Treino: {100*auc_train:.1f}%")
    plt.plot(roc_test[0], roc_test[1], linewidth=2, label=f"Teste: {100*auc_test:.1f}%")
    ...
    plt.title(f"Curva ROC - {model_final.__class__.__name__}", fontsize=14)
    ...
    # 6. LOG OPICIONAL: classification report se quiser detalhes
    ...
    # 7. LIMPEZA
    for file in ["feature_importances.csv", "roc_curve.png", "best_features_list.csv"]:
        if os.path.exists(file):
            os.remove(file)

    print(f"\n✅ Run finalizada: {grid_run.info.run_id}")
