...
def pod_academy_generate_metadata(df, ids, targets, orderby="PC_NULOS"):
    ...
    summary = pd.DataFrame(
        {
            "USO_FEATURE": [
                "ID" if col in ids else "Target" if col in targets else "Explicativa"
...
def avaliar_modelo(X_train, y_train, X_test, y_test, modelo, nm_modelo):
    ...
    if importancias is not None:
        ...
        axs[0, 1].barh(
            importancias_df["feature"], importancias_df["importance"], color=cor
        )
    ...
    axs[3, 0].bar(
        ["Treino", "Teste"], [gini_train, gini_test], color=[cor, "darkorange"]
    )
    axs[3, 0].set_title("Gini - " + nm_modelo)
    axs[3, 0].set_ylim(0, 1)
    axs[3, 0].text(
        "Treino", gini_train + 0.01, f"{gini_train:.2f}", ha="center", va="bottom"
    )
    axs[3, 0].text(
        "Teste", gini_test + 0.01, f"{gini_test:.2f}", ha="center", va="bottom"
    )
    ...
    axs[3, 1].text(
        "Treino", ks_train + 0.01, f"{ks_train:.2f}", ha="center", va="bottom"
    )
    ...
    axs[4, 1].bar(
        range(1, len(decile_analysis) + 1), decile_analysis, color="darkorange"
    )
    ...
    axs[4, 0].bar(
        range(1, len(decile_analysis_train) + 1), decile_analysis_train, color=cor
    )
